...
function EditNews() {
  ...
  const save = async () => {
    ...
    try {
      ...
      await axios.post("http://localhost:3000/api/newsitems/editnewsitem", payload);
      setLoading(false);
  ...
  const getData = async () => {
    setLoading(true);
    try {
      const result = await axios.post("http://localhost:3000/api/newsitems/getnewsitembyid", {
        newsid: params.newsid,
      });
